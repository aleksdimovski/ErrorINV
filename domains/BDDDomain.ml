...
module type BDDDomain = sig
  ...
  val vars : t -> var list  
  val nodes : t -> AbstractSyntax.bExp list 
...
(*  val init : unit -> Cudd.Man.v Cudd.Man.t*)
  ...
  val bot : string Bddapron.Env.t -> var list -> AbstractSyntax.bExp list -> t
  val top : string Bddapron.Env.t -> var list -> AbstractSyntax.bExp list -> t
  val inner : string Bddapron.Env.t -> var list -> AbstractSyntax.bExp list -> domain -> t
  ...
  val fwdAssign : t -> aExp * exp -> t
  val bwdAssign : t -> aExp * exp -> t  
  val filter : t -> bExp -> t
  val join : t -> t -> t
  val meet : t -> t -> t
  val widen : t -> t -> t
  ...
  val toBDD : Apron.Lincons1.earray -> t -> t
  val toProcess : t -> t -> (string Bddapron.Expr1.Bool.t * Apron.Lincons1.earray * Apron.Lincons1.earray) list
  val meetcond : t -> string Bddapron.Expr1.Bool.t -> t
