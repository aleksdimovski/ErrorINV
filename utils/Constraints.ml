...
sig
  ...
  val to_stringZ3 : var list -> t -> string -> string
  val to_stringZ3_bwd : var list -> t -> string -> string  
  val to_stringLatte : var list -> t -> (string*bool)
  ...
  let to_stringZ3 vars c name =
    ...
    let rec aux c x v =
      ...
      | Coeff.Scalar s -> 
        if v <> "" && Scalar.sgn s = 0 then () else (
          if Scalar.sgn s < 0 then 
            if v <> "" && Scalar.equal_int s (-1) then (if !first then str := !str ^ "(- 0 " ^ x ^ ")" else str := "(- " ^ !str ^ "" ^ x ^ ")") else
              (if !first then str := !str ^ "-" ^ (Scalar.to_string (Scalar.neg s)) ^ x else str := "(- " ^ !str ^ "" ^ (Scalar.to_string (Scalar.neg s)) ^ x ^ ")" )
          else if !first then 
            if v <> "" && Scalar.equal_int s 1 then str := !str ^ x else
              str := !str ^ (Scalar.to_string s)
          else 
          if v <> "" && Scalar.equal_int s 1 then str := "(+ " ^ !str ^ "" ^ x ^ ")" else
            (if !first then str := !str ^ "+" ^ (Scalar.to_string (Scalar.neg s)) ^ x else str := "(+ " ^ !str ^ "" ^ (Scalar.to_string (Scalar.neg s)) ^ x ^ ")" );
          if v <> "" then str := !str ^ v;
      ...
      | Coeff.Interval i ->
        if Scalar.equal i.Interval.inf i.Interval.sup then
          aux (Coeff.Scalar i.Interval.inf) x v
        else (
          if not !first then str := !str ^ "+" ^ x;
          str := !str ^ "[" ^ (Scalar.to_string i.Interval.inf) ^ "," ^ (Scalar.to_string i.Interval.sup) ^ "]" ^ x;
          if v <> "" then str := !str ^ v;
        );
        first := false
    in
    ...
    let k = Coeff.neg (Lincons1.get_cst c) in
    if !first then str := !str ^ "0";
    first := true; 
	let (front,back) = if (String.contains name 'B') then ("(not ",")") else ("","") in 
    (match Lincons1.get_typ c with
     | Lincons1.EQ -> str := "(assert (! "^front^"(= " ^ !str; aux k "" ""; str := !str ^ ")"^back^" :named " ^ name ^ "))"; !str
     | Lincons1.SUPEQ -> str := "(assert (! "^front^"(>= " ^ !str; aux k "" ""; str := !str ^ ")"^back^" :named " ^ name ^ "))"; !str
     | Lincons1.SUP -> str := "(assert (! "^front^"(> " ^ !str; aux k "" ""; str := !str ^ ")"^back^" :named " ^ name ^ "))"; !str
     | Lincons1.DISEQ -> raise (Invalid_argument "print:DISEQ")
     | Lincons1.EQMOD s -> raise (Invalid_argument "print:EQMOD"))



  let to_stringZ3_bwd vars c name =
    let str = ref "" in 
    let first = ref true in
    let rec aux c x v =
      match c with
    ...
    first := true; 
    (match Lincons1.get_typ c with
     | Lincons1.EQ -> str := "(= " ^ !str; aux k "" ""; str := !str ^ ")"; !str
     | Lincons1.SUPEQ -> str := "(>= " ^ !str; aux k "" ""; str := !str ^ ")"; !str
     | Lincons1.SUP -> "(> " ^ !str; aux k "" ""; str := !str ^ ")"; !str
     | Lincons1.DISEQ -> raise (Invalid_argument "print:DISEQ")
     | Lincons1.EQMOD s -> raise (Invalid_argument "print:EQMOD"))
     


  let to_stringLatte liveVars c =
  	let str = ref "" in 
    let first = ref true in
    let rec aux c x v =
      match c with
      | Coeff.Scalar s -> 
        if v <> "" && Scalar.sgn s = 0 then str := !str ^ " 0" else (
          if Scalar.sgn s < 0 then 
            (if v <> "" then str := !str ^ " -" ^ (Scalar.to_string (Scalar.neg s)) else str := "-" ^ (Scalar.to_string (Scalar.neg s)) ^ !str)
          else 
            if v <> "" then str := !str ^ " " ^ (Scalar.to_string s) else str := (Scalar.to_string s) ^ !str
        )
      | Coeff.Interval i ->
        if Scalar.equal i.Interval.inf i.Interval.sup then
          aux (Coeff.Scalar i.Interval.inf) x v
    ...
    Lincons1.iter (fun v x -> 
        try 
          let x = List.find (fun y -> String.compare (Var.to_string x) y.varId = 0) liveVars in
          (*str := !str ^ (x.varName);*)
          aux v x.varName (" ")
        with Not_found -> ()
      ) c;
    ...
    first := true;
    (match Lincons1.get_typ c with
     | Lincons1.EQ -> aux k "" ""; (!str,true)
     | Lincons1.SUPEQ -> aux (Coeff.neg k) "" ""; (!str,false)
     | Lincons1.SUP -> aux (Coeff.neg k) "" ""; (!str,false)
     | Lincons1.DISEQ -> raise (Invalid_argument "print:DISEQ")
     | Lincons1.EQMOD s -> raise (Invalid_argument "print:EQMOD"))
